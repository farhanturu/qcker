...
pub struct CreateArgs {
    ...
    #[arg(long, help = "CPU cores (e.g., 1.5)")]
    cpus: Option<f64>,

    #[arg(long, help = "CPU shares (default 1024)")]
    cpu_shares: Option<u64>,

    #[arg(short = 'm', long, help = "Memory limit in MB")]
    memory: Option<u64>,

    #[arg(long, help = "Memory + swap limit in MB")]
    memory_swap: Option<u64>,

    #[arg(long, help = "Max processes (default 256)")]
    pids_limit: Option<i64>,

    #[arg(long, help = "Enable GPU")]
    gpu: bool,

    #[arg(long, help = "GPU devices")]
    ...
    #[arg(long, help = "Privileged mode")]
    privileged: bool,
}

pub fn execute(args: CreateArgs, data_dir: &Path, format: &str) -> anyhow::Result<()> {
        ...
        || resource_limits.gpu_enabled
    {
        ...
        output::print_success(&format!("Resource limits applied: CPU={:?} cores, Memory={:?} MB, GPU={}",
            resource_limits.cpu_cores, resource_limits.memory_mb, resource_limits.gpu_enabled));
    }

    output::print_success(&format!("Container {} created", container_id));
    output::print_container_state(&container_id, "created", None, format);
